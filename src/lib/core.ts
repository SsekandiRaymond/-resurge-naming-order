...
export  function NONamesFC(countryCode: ISO2Codes): CountryInfo {
    if(typeof countryCode !== "string") throw new Error("The parameter (countryCode) must be a 2 alpha-character string")
    let countryInfo = json.find((c) => c.code.toUpperCase() === countryCode.toUpperCase());
    if (!countryInfo) throw new Error("Country Code passed is not recognised", {cause: countryCode.length> 2? "Passed a non-ISO alpha 2 country code": "The passed countryCode is not recorded in package core.json, please rich out to the author for update"})
